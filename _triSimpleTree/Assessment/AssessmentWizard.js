...
$(document).ready(function () {
    ...
    $(".next").click(function () {
        ...
        if (typeof (next_fs[0]) === "undefined") {
            ...
            currentQuestionCategoryId = next_fs.attr('sectionid');
            var liElement = document.getElementById(currentQuestionCategoryId);
            liElement.className = "easingactive";
    ...
    $("#easingprogressbar li").on('click', function () {
		var sectionIdQuesCAtgId = $(this).attr('id');
        ProgressLiClickHandler(animating, left, opacity, scale, sectionIdQuesCAtgId,true);
    });

	$("#topFixedNavbar1 ul li:not(:first-child)").on('click',function(){
		var sectionIdQuesCAtgId = $(this).attr('section-id');
		CollapseControl();
		MakeHeight50px();	
		ProgressLiClickHandler(animating, left, opacity, scale, sectionIdQuesCAtgId,false);
	});
	
	$("#hamburger").on('click',function(){
		
		//var topFixedNavbar1 = $('#topFixedNavbar1');
		if($(this).hasClass('collapsed')){			
			setTimeout(function(){RemoveHeight50px();},10);	
		}else{
			setTimeout(function(){MakeHeight50px();},30);	
			//MakeHeight50px();
		}
...
function SetCurrentQuestionCategoryId() {
    var vCurrentLi = $("#easingprogressbar li.easingactive");//.find('.active');
    ...
    req.onreadystatechange = function () {
        if (this.readyState === 4) {
            ...
            if (this.status === 200) {
                ...
                // Clear the array
                sections = [];
				var categoriesLength = results.value.length; 
                for (var i = 0; i < results.value.length; i++) {
                    ...
                    if (i === 0) {
                        $("#easingprogressbar").append('<li class="easingactive" id="' + tri_assessmentquestioncategoryid + '" sectionNo="'+(i+1)+'" sectionName="'+tri_name+'">'+'<div class="liElement">' + tri_name + '</div></li>');
                    }
                    else {
                        //$("#progressbar").append('<li id="' + tri_assessmentquestioncategoryid + '">' + tri_name + '</li>');
                        $("#easingprogressbar").append('<li id="' + tri_assessmentquestioncategoryid + '" sectionno="'+(i+1)+'" sectionName="'+tri_name+'">'+ '<div class="liElement">' + tri_name + '</div></li>');
                    }
					
					var txt = "Part " + (i+1) + ": " + tri_name;
					var li = document.createElement('li');
					var anchr = document.createElement('a');
					li.setAttribute('section-id',tri_assessmentquestioncategoryid);
					li.setAttribute('sectionName',tri_name);
					li.setAttribute('sectionno',i+1);
					anchr.className = "hidden-on-desktop";
					anchr.href="#";
					anchr.innerHTML = txt;
                    ...
                    GetAssesmentQuestions(tri_name, _tri_assessmenttypeid_value, tri_assessmentquestioncategoryid,categoriesLength);
                }
				// Add li score li element
				var liScore = document.createElement('li');
				var anchrScore = document.createElement('a');
				liScore.setAttribute('id','liScore');
				anchrScore.className = "hidden-on-desktop";
				anchrScore.href="#";
				anchrScore.setAttribute('style','font-size: 1.1em; font-family: Segoe UI; font-weight: 600;');
				
				liScore.appendChild(anchrScore);
				docFrag.appendChild(liScore);
				
				$("#topFixedNavbar1 ul").append(docFrag);
            }
            else {
                        ...
                        if (j == results.value.length || i == results.value.length) {
							...
							if (firstView === false && categoryLength == 1) {
                                ...
                                if (isLastCategory === true) {
                                    ...
                                    closeButton = '<div name="close" class="closeable button default-size navy-bg">Close</div>&nbsp;';
        ...
        if (this.readyState === 4) {
            ...
            if (this.status === 200) {
                ...
                if (statusCode == assessmentIsComplete) {
                    ...
                    $('#divScore').show();
					//$('#liScore a').text('Score: ' + overallScore);
                } else {
                    $('#divScore').hide();
					//$('#liScore a').text('');
                }
...
function buildScoreWindow(results) {
    ...
    scorehtml += "</table><br/>";
    scorehtml += "<input type='button' name='closescore' class='action-button' value='closeable Window' onclick='window.close();'/>";
    scorehtml += "</div>";
...
function updateClassForLIElementActiveTill(clickedElementId) {
    ...
    var elements = $("#easingprogressbar li");
    for (var i = 0 ; i < elements.length; i++) {
        curElementId = elements[i].id;
        if (bAddActiveClass) {
            if (elements[i].className !== "easingactive") {
                elements[i].className = "easingactive";
            }
        } else {
            if (elements[i].className === "easingactive") {
...
function ProgressLiClickHandler(animating, left, opacity, scale, sectionIdQuesCatgId, updateProgressBar){
	try {
            if (animating) {
                return false;
            }
			...
			var liEle;
			if(typeof sectionIdQuesCatgId === "undefined" || sectionIdQuesCatgId === null){
				return;
			}
			...
			//vClickedLIid = liEle.attr('id');
			vClickedLIid=sectionIdQuesCatgId;
			...
			/*if(updateProgressBar){
				liEle = $("#"+sectionIdQuesCatgId);
				makeLiElementActiveTill(liEle);	
			}*/			
			liEle = $("#"+sectionIdQuesCatgId);
			makeLiElementActiveTill(liEle);
...
function CollapseControl(){
	...
	var element = $('#topFixedNavbar1');
	
	if(element.hasClass('in')){
		element.removeClass('in');
		element.attr('aria-expanded','false');
		var sibling = element.siblings();
		if(!sibling.children('button').hasClass('collapsed')){
			sibling.children('button').addClass('collapsed');
			sibling.children('button').attr('aria-expanded','false');
			
		}
	}
}

function RemoveHeight50px(){
	$('#fluidcontainer').css({height:''});
}
function MakeHeight50px(){
	//$('#fluidcontainer').css({height:'50px'});
	$('#fluidcontainer').animate({height:'50px'});
...
function SetOverallScore(){
	...
	if(typeof overallScore !== "undefined" && overallScore !== ""){
		$('#liScore a').text('Score: ' + overallScore);	
	}else{
		$('#liScore a').text('Score: ');
	}
